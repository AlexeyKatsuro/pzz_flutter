...
List<Middleware<AppState>> createPzzMiddleware(
  ...
  return [
    TypedMiddleware<AppState, InitialAction>(
      _createInitial(pzzRepository, preferenceRepository),
    ),
    TypedMiddleware<AppState, LoadPizzasAction>(
      _createLoadPizzas(pzzRepository),
    ),
    TypedMiddleware<AppState, LoadBasketAction>(
      _createLoadBasket(pzzRepository),
    ),
    TypedMiddleware<AppState, AddProductAction>(
      _createAddPizzaItem(pzzRepository),
    ),
    TypedMiddleware<AppState, RemoveProductAction>(
      _createRemovePizzaItem(pzzRepository),
    ),
    TypedMiddleware<AppState, SavePersonalInfoAction>(
      _createSavePersonalInfo(preferenceRepository),
    ),
    TypedMiddleware<AppState, LoadHousesAction>(
      _createLoadHouses(pzzRepository, preferenceRepository),
    ),
    TypedMiddleware<AppState, SelectHouseAction>(
      _createSelectHouse(preferenceRepository),
    ),
    TypedMiddleware<AppState, TryPlaceOrderAction>(
      _createUpdateAddress(pzzRepository),
    ),
    TypedMiddleware<AppState, ConfirmPlaceOrderAction>(
      _createConfirmPlaceOrder(pzzRepository),
    ),
  ];
}

MiddlewareTyped<AppState, InitialAction> _createInitial(
  PzzRepository repository,
  PreferenceRepository preferenceRepository,
) {
  return (
...
MiddlewareTyped<AppState, LoadPizzasAction> _createLoadPizzas(
  PzzRepository repository,
) {
  return (
    Store<AppState> store,
    ...
    repository.loadPizzas().then((pizzas) {
      ...
      next(
        SetScopedErrorAction(
          error: errorMessageExtractor(ex),
          scope: action.scope,
        ),
      );
    });
...
MiddlewareTyped<AppState, LoadBasketAction> _createLoadBasket(
  PzzRepository repository,
) {
...
MiddlewareTyped<AppState, AddProductAction> _createAddPizzaItem(
  PzzRepository repository,
) {
...
MiddlewareTyped<AppState, RemoveProductAction> _createRemovePizzaItem(
  PzzRepository repository,
) {
  return (
    Store<AppState> store,
...
MiddlewareTyped<AppState, SavePersonalInfoAction> _createSavePersonalInfo(
  PreferenceRepository repository,
) {
  return (
    Store<AppState> store,
    SavePersonalInfoAction action,
    NextDispatcher next,
  ) {
...
MiddlewareTyped<AppState, SelectHouseAction> _createSelectHouse(
  PreferenceRepository repository,
) {
  return (
    Store<AppState> store,
    SelectHouseAction action,
    NextDispatcher next,
  ) async {
    next(action);
    //repository.savePersonalInfo(personalInfoSelector(store.state));
...
MiddlewareTyped<AppState, TryPlaceOrderAction> _createUpdateAddress(
  PzzRepository repository,
) {
  return (
    Store<AppState> store,
    TryPlaceOrderAction action,
    NextDispatcher next,
  ) async {
    next(action);
...
MiddlewareTyped<AppState, ConfirmPlaceOrderAction> _createConfirmPlaceOrder(
  PzzRepository repository,
) {
  return (
    Store<AppState> store,
    ConfirmPlaceOrderAction action,
    NextDispatcher next,
  ) async {
    next(action);
    next(ConfirmLoadingAction(isLoading: true));
    repository.placeOrder().then((basket) {
      next(
        NavigateAction.pushAndRemoveUntil(
          Routes.successOrderPlacedDialog,
          untilPage: Routes.homeScreen,
          arguments: 60,
        ),
      );
